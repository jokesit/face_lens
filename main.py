...
class RecognitionThread(QThread):
    ...
    def __init__(self):
        super().__init__()
        self.recognizer = None; self.faiss_index = None; self.index_to_name_map = {}
        self.RECOGNITION_THRESHOLD = 0.75; self.is_running = True
    ...
    def build_faiss_index(self, db_data):
        try:
            ...
            print(f"RecognitionThread: Faiss index built with {self.faiss_index.ntotal} vectors.")
    ...
    def process_verification_job(self, captured_embeddings, name):
        if not captured_embeddings or self.recognizer is None:
            self.verification_result.emit(name, -1.0); return
        
        new_avg = np.mean(captured_embeddings, axis=0)
        new_avg /= np.linalg.norm(new_avg)
        
        db = Database()
        existing = db.get_customer_by_name(name)
        distance = -1.0 # ใช้ -1.0 เป็นค่าเริ่มต้นสำหรับ "ลูกค้าใหม่"
...
class VideoThread(QThread):
    ...
    def __init__(self):
        ...
        self.is_running = True; self.frame_counter = 0; self.RECOGNITION_INTERVAL = 20
        self.last_known_results = {}
        self.is_capture_mode = False; self.capture_count = 0
        self.last_face_box = None; self.capture_cooldown = 0
        self.COOLDOWN_FRAMES = 30; self.MIN_FACE_MOVEMENT = 10; self.MAX_SNAPSHOTS = 5

    @Slot(str, object)
    def update_recognition_results(self, name, box): self.last_known_results[box] = name
        
    @Slot(bool)
    def set_capture_mode(self, is_active):
        self.is_capture_mode = is_active
        if is_active: self.capture_count = 0
        else: self.last_face_box = None
        ...
        while self.is_running:
            ...
            if self.is_capture_mode:
                ...
                if self.frame_counter % self.RECOGNITION_INTERVAL == 0:
                    self.last_known_results = {}
                    if faces: self.recognition_job_signal.emit(faces[0], boxes[0])
                    else: self.update_display_name_signal.emit("Unknown")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("FaceLens (Pro Architecture)"); self.setGeometry(100, 100, 800, 720)
        self.setWindowIcon(QIcon("assets/logo.png")); self.db = Database()
        ...
        self.recognition_thread.start(); self.video_thread.start()
    ...
    def open_add_customer_dialog(self):
        ...
        self.recognition_thread.new_embedding_captured.connect(dialog.add_captured_embedding)
        ...
        self.recognition_thread.new_embedding_captured.disconnect(dialog.add_captured_embedding)
    ...
    @Slot()
    def on_customer_saved(self): self.rebuild_index_signal.emit(self.db.get_all_data_for_faiss())
        
    def closeEvent(self, event): self.video_thread.stop(); self.recognition_thread.stop(); event.accept()
        
    @Slot(QPixmap)
    def update_image(self, pixmap): self.image_label.setPixmap(pixmap)
